...
class RegistrationFormService {
   private formatFormDetail<
      T extends {
         _count: { registrationResponses: number };
         subEvent: { eventId: string };
      },
   >(detail: T) {
      const { _count, subEvent, ...form } = detail;
      void subEvent;

      return {
         ...form,
         responseCount: _count.registrationResponses,
      };
   }

   private async assertCanViewForm(
      eventId: string,
      user: typeof auth.$Infer.Session.user,
   ) {
      if (isAdminUser(user)) return;

      await eventCommitteeService.assertEventCommitteeMember(eventId, user.id);
   }

   private validateFormForPublishing(
      form: Awaited<
         ReturnType<typeof registrationFormRepository.findFormDetailById>
      >,
   ) {
      if (!form) {
      ...
      const activeQuestions = form.questions.filter(
         (question) => question.status === 'ACTIVE',
      );
      const fieldKeys = activeQuestions.map((question) => question.fieldKey);

      if (new Set(fieldKeys).size !== fieldKeys.length) {
         throw new AppError('Active question field keys must be unique', 400);
      }

      const orderIndexes = activeQuestions
         .map((question) => question.orderIndex)
         .sort((left, right) => left - right);
      const hasInvalidOrder = orderIndexes.some(
         (orderIndex, index) => orderIndex !== index,
      );

      if (hasInvalidOrder) {
         throw new AppError(
            'Active question order indexes must be sequential',
            400,
         );
      }
      ...
      for (const question of activeQuestions) {
         if (
            optionFieldTypes.includes(question.fieldType) &&
            !question.options.some((option) => option.isActive)
         ) {
            throw new AppError(
   ...
   async createRegistrationForm(
      ...
      const subEvent =
         await registrationFormRepository.findSubEventById(subEventId);
      ...
      const existingForm =
         await registrationFormRepository.findFormBySubEventId(subEvent.id);

      if (existingForm) {
         throw new AppError(
            'A registration form already exists for this sub-event',
            409,
         );
      }

      const form = await registrationFormRepository.createForm(
         subEvent.id,
         user.id,
      );

      return this.formatFormDetail(form);
   }

   async getRegistrationFormBySubEventId(
      subEventId: string,
      user: typeof auth.$Infer.Session.user,
   ) {
      const subEvent =
      ...
      await this.assertCanViewForm(subEvent.eventId, user);

      const form = await registrationFormRepository.findFormBySubEventId(
         subEvent.id,
      );
   ...
   async getRegistrationFormById(
      id: string,
      user: typeof auth.$Infer.Session.user,
   ) {
      const form = await registrationFormRepository.findFormDetailById(id);

      if (!form) {
         throw new AppError('Registration form not found', 404);
      }

      await this.assertCanViewForm(form.subEvent.eventId, user);

      return this.formatFormDetail(form);
   }

   async updateRegistrationFormStatus(
      payload: UpdateRegistrationFormStatusRequest,
      id: string,
      user: typeof auth.$Infer.Session.user,
   ) {
      const form = await registrationFormRepository.findFormDetailById(id);
      ...
      if (currentStatus === 'CLOSED') {
         throw new AppError(
            'Closed registration forms cannot be reopened',
            400,
         );
      }
      ...
      if (nextStatus === 'PUBLISHED') {
         if (currentStatus !== 'DRAFT') {
            throw new AppError('Only draft forms can be published', 400);
         }
      ...
      if (currentStatus === 'PUBLISHED' && nextStatus === 'DRAFT') {
         if (form._count.registrationResponses > 0) {
            throw new AppError(
               'Cannot unpublish a form after responses exist',
               400,
            );
         }

         const updatedForm =
            await registrationFormRepository.updateFormStatusAndCloseRegistration(
               form.id,
               form.subEventId,
               nextStatus,
               user.id,
         ...
         return this.formatFormDetail(updatedForm);
      }
      ...
      const updatedForm = await registrationFormRepository.updateFormStatus(
         form.id,
         nextStatus as RegistrationFormStatus,
         user.id,
      );
